// automate_trading/workers/inventory_worker.js
 ...
 * - Updates orchestration tasks and negotiation JSONs
 */
class InventoryWorker {
  ...
  */
  ...
  /**
   * ✅ RESTORED: Update orchestration task and negotiation JSON with results
   */
  async updateTaskResults(task, inventoryResult, accountSteamId, targetSteamId) {
    try {
      ...
      // Update negotiation JSON second
      try {
        this.logger.info(`${this.workerName}: Updating negotiation JSON for ${task.taskID}; targetID: ${targetSteamId}`);
        await this.updateNegotiationJson(accountSteamId, targetSteamId, inventoryResult);
        this.logger.info(`${this.workerName}: Successfully updated negotiation JSON for ${task.taskID}`);
      } catch (error) {
  ...
  async updateOrchestrationTask(task, inventoryResult) {
    try {
      ...
      const contextUpdates = {
        [inventoryKey]: inventoryResult.inventory,
  ...
  /**
   * ✅ RESTORED & FIXED: Update negotiation JSON with inventory data
   * Uses HttpClient loadNegotiations/saveNegotiations methods correctly
   */
  async updateNegotiationJson(accountSteamId, targetSteamId, inventoryResult) {
    try {
      // Debug logging for troubleshooting
      this.logger.info(`${this.workerName}: updateNegotiationJson called with:`, {
        accountSteamId,
        targetSteamId,
        inventoryResult: {
      ...
      this.logger.info(`${this.workerName}: Saving updated negotiations to HttpClient`);
      ...
      this.logger.info(`${this.workerName}: Successfully updated negotiation JSON for ${isOurInventory ? 'our' : 'friend'} inventory`);
      
    } catch (error) {
      this.logger.error(`${this.workerName}: Error updating negotiation JSON: ${error.message}`);
      this.logger.error(`${this.workerName}: Stack trace:`, error.stack);
      throw error;
    }
  }
  ...
  /**
   * Check if auth token failure is due to disconnected Steam session
   */
  async checkSteamConnectionStatus(ourSteamId) {
    try {
      const allSessions = this.sessionManager.listActiveSessions();
      const session = allSessions.find(s => s.steamId === ourSteamId);
      
      if (!session) {
        return {
          connected: false,
          reason: 'No session found in SteamSessionManager',
      ...
      if (session.sessionAge > 600) { // 10 minutes
        return {
          connected: 'stale',
          reason: `Session is stale (${session.sessionAge}s old)`,
          shouldReconnect: true,
          sessionAge: session.sessionAge
        };
      }
      ...
      this.logger.warn(`Error checking Steam connection status: ${error.message}`);
      return {
        connected: 'unknown',
        reason: 'Failed to check session manager',
        shouldReconnect: false
      };
    }
  }
